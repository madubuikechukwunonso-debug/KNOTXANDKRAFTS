...
export const paymentRouter = createRouter({
  createProductCheckoutSession: publicQuery
    ...
    .mutation(async ({ input }) => {
      ...
      const successBase = requiredUrl("APP_URL");
      const cancelBase = requiredUrl("APP_URL");

      const session = await stripe.checkout.sessions.create({
        mode: "payment",
        customer_email: input.customerEmail,
        success_url: `${successBase}/checkout?success=1&orderId=${orderId}`,
        cancel_url: `${cancelBase}/cart?cancelled=1`,
  ...
  createServiceCheckoutSession: publicQuery
    ...
    .mutation(async ({ input }) => {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: `${successBase}/booking?paid=1&bookingId=${booking.id}`,
        cancel_url: `${cancelBase}/booking?cancelled=1&bookingId=${booking.id}`,
